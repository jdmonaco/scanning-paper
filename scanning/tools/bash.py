...
# Library imports
from traits.api import HasTraits, Instance, Trait, String, Bool
...
try:
    ...
    notifier_imported = False
else:
    notifier_imported = True
...
class CPrint(HasTraits):
    ...
    timestamp -- if logging to file, set this to control whether
        timestamps are printed or not
    ...
    def _notifier_default(self):
        return Notifier(log=False)

    def __call__(self, s, prefix=None, error=False, popup=False):
        ...
        if _plat == 'win32':
            ...
            msgc = error and lightred or white
        ...
        if popup and notifier_imported:
            if error:
                self.notifier.notify(first_cap(s.strip()), pre[:-2], "Error")
            else:
                self.notifier.notify(first_cap(s.strip()), pre[:-2])
